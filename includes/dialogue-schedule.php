...
function mith_dialogues_schedule_shortcode($atts) {
	...
	$array_rev = array_reverse($current_dds->posts, true); //reverse the order of the posts, latest last
	...
	$dd_content .= '<h2 class="post-title dialogues-series-title dialogues-header">' . $current_cat_posts->name . __( ' Digital Dialogues Schedule', 'Avada' ) . '</h2>
		<form id="dialogue_series_form" action="' . esc_url( home_url( '/' ) ) . '/" method="get">
			<div class="dialogue-series-form">' . 
				restrict_dialogues_by_series() . '
				<input type="submit" name="submit" value="view" />
			</div>
		</form>';
					  
	if ( $current_dds->have_posts() ) :
		$dd_content .= do_shortcode('[separator style_type="double" top_margin="20" bottom_margin="20" sep_color="#BBBBBB" border_size="" width="" alignment="" class="" id=""]'); 
		$dialogues_content .= '
	...
	$dd_content .= $diddalogues_content;
	$html = $dd_content; 	
	return $html;
}

add_shortcode( 'mith_dialogues_schedule', 'mith_dialogues_series_shortcode' );
